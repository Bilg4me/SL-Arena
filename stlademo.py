...
def charge_arene(aid):
    equipeNumeros = [None for i in range(10)]
    resultatsSQL = curseur.execute("SELECT pid, emplacement FROM arenes WHERE aid = "+str(aid))
    for ligne in resultatsSQL:
        equipeNumeros[ligne[1]] = ligne[0]
    return equipeNumeros
...
def lance_jeu():
    ...
    etatJeu = EtatJeu()
    equipeJoueurNumeros = [None, 1, None, 2, None, 3, None, 4, None, 6]
    etatJeu.equipes[0] = initialise_equipe(equipeJoueurNumeros, EQ_JOUEUR)
    aid = 0
    statut = EJ_CHARGEMENT_ARENE
    ...
    while enCours:
        ...
        if statut == EJ_CHOIX_EQUIPE:
            ...
            if resultat == VICTOIRE_J2 or resultat == EGALITE:
                ...
                if etatJeu.doitJouer.equipe == EQ_ADVERSAIRE:
                    i = randrange(3)
                    while etatJeu.doitJouer.capacites[i].attente != 0:
                        i = randrange(3)
        ...
        elif statut == EJ_CHARGEMENT_ARENE:
            aid += 1
            statut = EJ_CHANGE_TOUR
            etatJeu.equipes[1] = initialise_equipe(charge_arene(aid), EQ_ADVERSAIRE)
            continue
