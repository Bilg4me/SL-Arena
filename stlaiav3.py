...
@updates: on essaie de prevoir plusieurs coup à l'avance
...
def tour_de_jeu(etatJeu, memo):
	if memo == None:
		memo = [0, decisionCoup(etatJeu)]
		
	if memo[0] == 0:
		coupJoué = memo[1][0] # premier coup
	elif memo[0] == 1:
		coupJoué = memo[1][1] # deuxiemeCoup
		memo[0] = 0
		memo[1] = decisionCoup(etatJeu)
	memo[0] = (memo[0] + 1) % 2
	return coupJoué + (memo,)
...
def respectRegles(etatJeu):
	...
	return (JoueurAttaquable_emplacement,JoueurAlliés_emplacement, capacitesDispo_emplacement)
...
def simulation(etatJeu, emplacementCibleAdverse ,emplacementCibleAlliée, capacite_emplacement):
	...
	return score(etatSimulé),etatSimulé

def meilleurTourSuivant(etatJeu):
	(JoueurAttaquable_emplacement, JoueurAlliés_emplacement, capacitesDispo_emplacement) = respectRegles(etatJeu)
	coupsPossibles = product(JoueurAttaquable_emplacement,JoueurAlliés_emplacement, capacitesDispo_emplacement)
	
	return max([ (simulation(etatJeu,adversaire,allié,capacite)[0],(adversaire,allié,capacite)) for (adversaire,allié,capacite) in coupsPossibles])
...
def decisionCoup(etatJeu):
	...
	coupsPossibles = product(JoueurAttaquable_emplacement,JoueurAlliés_emplacement, capacitesDispo_emplacement)
	coupsJoués = []
	meilleurScore,_ = -1,(None,None,None)
	
	for adversaire,allié,capacite in coupsPossibles:
		etatSimulé = simulation(etatJeu, adversaire,allié,capacite)[1]
		score,deuxiemeCoup = meilleurTourSuivant(etatSimulé)
		
		if score > meilleurScore:
			meilleurScore = score
			coupsJoués = [(adversaire,allié,capacite), deuxiemeCoup]
